...
import           Formula              (Chc, Expr ((:@)), Type ((:=>)), Var (..))
import qualified Formula              as F
import           Z3.Monad

data Env = Env
  { _envVars :: Map Var AST
...
treeInterpolate ::
     MonadIO m => Tree Expr -> m (Either (Map Var Expr) (Tree Expr))
treeInterpolate ftree =
  runEnvZ3 $
  ...
  -- interpolant `false` if the input is unsatisfiable).
   do
    ...
    case is of
      Just (Right is') ->
        Right <$> (buildTree <$> traverse astToExpr is' <*> pure ftree)
      Just (Left m) -> Left <$> modelToModel m
      _ -> pure (Left M.empty)
...
solveChc hcs = do
  ...
  where
    ...
    mkQuery q n =
      let theQuery = F.V $ F.Var n F.Bool
      in F.app2 F.Impl (F.mkNot $ F.chcExpr q) theQuery
    useDuality = do
      pars <- mkParams
      join $
        paramsSetSymbol pars <$> mkStringSymbol "engine" <*>
        mkStringSymbol "duality"
...
isValid f =
  runEnvZ3 $ do
    ...
    case sol of
      ...
      _ -> return False
  where
    sc = (assert =<< formToAst (F.mkNot f)) >> check
...
superSimplify f = runEnvZ3 $ astToExpr =<< superSimp =<< formToAst f
  where
    ...
    superSimp ast = do
      ...
      tac' <-
        join $ andThenTactic <$> mkTactic "ctx-solver-simplify" <*> pure tac
...
newtype EnvZ3 n a = EnvZ3
  { getEnvZ3 :: StateT Env Z3 a
...
appToZ3 f args =
  ...
  where
    ...
    three o =
      join $
      o <$> formToAst (head args) <*> formToAst (args !! 1) <*>
      formToAst (args !! 2)
...
formFromApp n args range
  ...
  where
    lift2 f = F.app2 f <$> astToExpr (head args) <*> astToExpr (args !! 1)
    liftMany f = F.appMany f <$> traverse astToExpr args

-- | Convert a Z3 model to the AST-based formula model.
modelToModel :: (MonadState Env z3, MonadZ3 z3) => Model -> z3 F.Model
...
astToExpr arg = do
  ...
  case k of
    ...
    Z3_APP_AST -> do
      ...
      formFromApp n args range
    Z3_VAR_AST ->
      F.V <$> (F.bound <$> (toInteger <$> getIndexValue arg) <*> getType arg)
    Z3_QUANTIFIER_AST -> do
      liftIO $ putStrLn "quantifier!"
      undefined
    Z3_SORT_AST -> do
      liftIO $ putStrLn "sort!"
      undefined
    Z3_FUNC_DECL_AST -> do
      liftIO $ putStrLn "func decl!"
      undefined
    Z3_UNKNOWN_AST -> do
